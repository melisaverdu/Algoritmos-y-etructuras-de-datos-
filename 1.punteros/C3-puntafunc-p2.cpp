...
// Programa de ordenamiento burbuja, multiprop�sito, mediante el uso de apuntadores a funciones.
#include <cstdlib>
#include <iostream>
#include <iomanip>
...
{  const int tamanoArreglo = 10;
   ...
   if ( orden == 1 ) {
      burbuja( a, tamanoArreglo, ascendente ); //CUANDO LLAMAMOS A LA FUNCION BURBUJA, LE PASAMOS EL PUNTERO A LA FUNCION ASCENDENTE, UNA DIRECCION A DONDE ESTA ESA FUNCION, POR ESO NO SE USAN PARENTESIS
...
void burbuja( int trabajo[], const int tamano, bool (*compara)( int, int ) )
{
   ...
   for ( int pasada = 1; pasada < tamano; pasada++ )
      ...
      for ( int cuenta = 0; cuenta < tamano - 1; cuenta++ )
         ...
         if ( (*compara)( trabajo[ cuenta ], trabajo[ cuenta + 1 ] ) )intercambia( &trabajo[ cuenta ], &trabajo[ cuenta + 1 ] );
         //coompara es un puntero
